...
function TimeTableMultiComponent() {
        ...
        <table id={weeklySchedule}>
          ...
          <tbody>
            {uniqueSubjects.sort().map((subject) => {
              const entriesForSubject = timetableData.filter(item => item.subject === subject);
              
              return (
                <tr key={subject}>
                  <td>{subject}</td>
                  {daysOfWeek.map(day => {
                    const entriesForDay = entriesForSubject.filter(item => item.day === day);
...
export default TimeTableMultiComponent;
